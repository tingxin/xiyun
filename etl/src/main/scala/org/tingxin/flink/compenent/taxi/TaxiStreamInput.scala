...
trait TaxiStreamInput extends FlinkInput {
  ...
  override def input(): Unit = {
    ...
    var input: DataStream[String] = env.readTextFile(files(0))
    for (i <- 1 until files.length) {
      val newFileStream: DataStream[String] = env.readTextFile(files(i))
      input = input.union(newFileStream)
    }

    val inValidInputTag: OutputTag[String] = OutputTag[String](id = "bad_input")
    val stream = input.process(new ProcessFunction[String, TripRecord] {
      override def processElement(i: String, context: ProcessFunction[String, TripRecord]#Context, collector: Collector[TripRecord]): Unit = {
        val fields = i.split(",")
        if (fields.length != 18) {
          context.output(inValidInputTag, s"fields count is wrong, $i")
        }
        if (fields(0) != "VendorID") {
          ...
          try {
            ...
            case e: Exception => context.output(inValidInputTag, s"${e.toString}, $i")
          }
        }
      }
    })

    val invalidDs = stream.getSideOutput(inValidInputTag)
    setDataSet(TAXI_INPUT_DS, stream)
    setDataSet(TAXI_INVALID_INPUT_DS, invalidDs)
  }
